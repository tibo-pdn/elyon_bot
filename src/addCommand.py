...
async def addCommand(message, argv):
    cursor.execute("SELECT uuid FROM user WHERE uuid = '" + str(message.author.id) + "';")
    result = cursor.fetchone()
    print("Add command")

    if result is None :
        cursor.execute("INSERT INTO user (uuid, u_gear, u_level, u_skill, r_red, r_orange, r_yellow, r_blue, r_green, r_purple) VALUES ('" + str(message.author.id) + "', 1, 2, 3, 4, 5, 6, 7, 8, 9);")
        connection.commit()
